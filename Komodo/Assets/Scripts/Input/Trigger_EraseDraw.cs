...
public class Trigger_EraseDraw : MonoBehaviour
{
    public void OnTriggerEnter(Collider other)
    {
        ...
        if(netReg)
        {
            ...
            if (ClientSpawnManager.Instance._EntityID_To_NetObject.ContainsKey(netReg.entity_data.entityID))
            {
                Destroy(ClientSpawnManager.Instance._EntityID_To_NetObject[netReg.entity_data.entityID].gameObject);
                ClientSpawnManager.Instance._EntityID_To_NetObject.Remove(netReg.entity_data.entityID);
            }

            NetworkUpdateHandler.Instance.DrawUpdate(
                   //    ClientSpawnManager.Instance.Draw_Refresh(
                   new Draw((int)ClientSpawnManager.Instance._mainClient_entityData.clientID, netReg.entity_data.entityID 
